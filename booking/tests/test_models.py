...
class BookingTest(TestCase):
    ...
    def test_booking_creation(self):
        ...
        now = datetime.now()
        booking = Booking.objects.create(boat=boat, renter=owner, start_date=date(now.year, 1, 1), end_date=date(now.year, 1, 10), total_sum=1000)
        self.assertEqual(str(booking), 'Бронь № %s - Boat1 (Лодка)' % booking.pk) 

        with self.assertRaises(BookingDateRangeException):
            booking.status = Booking.Status.ACCEPTED
            booking.save()
            Booking.objects.create(boat=boat, renter=owner, start_date=date(now.year, 1, 2), end_date=date(now.year, 1, 3), total_sum=200)
