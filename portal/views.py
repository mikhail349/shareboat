...
def portal(request):
    try:
        ...
        p = Paginator(articles, settings.PAGINATOR_ARTICLE_PER_PAGE).get_page(q_page)
        return render(request, 'portal/portal.html', context={'category': category, 'articles': p.object_list, 'p': p}) 

    except Category.DoesNotExist:
        try:
            article = Article.objects.published().get(full_path=request.path)
            return render(request, 'portal/article.html', context={'article': article}) 
