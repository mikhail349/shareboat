...
def search_boats(request):
    ...
    q_price_from = Decimal(request.GET.get('priceFrom')
                           ) if request.GET.get('priceFrom') else None
    q_price_to = Decimal(request.GET.get('priceTo')
                         ) if request.GET.get('priceTo') else None
    ...
    if q_state:
        boats = boats.filter(coordinates__state=q_state).union(
            boats.filter(base__state=q_state))
...
def create_or_update(request, pk=None):
    ...
    base = Base.objects.get(pk=data.get('base')) if data.get(
        'base') and not is_custom_location else None

    try:
        term = Term.objects.get(pk=data.get(
            'term'), user=request.user) if data.get('term') else None
        ...
        try:
            ...
            errors.append(ValidationError(
                'Модель не найдена', code="invalid_model"))
        ...
        with transaction.atomic():
            ...
            if boat.is_comfort_boat():
                try:
                    ...
                    comfort_boat.extra_berth_amount = data.get(
                        'extra_berth_amount')
                    ...
                    ComfortBoat.objects.create(
                        **comfort_boat_fields, boat=boat)
            else:
