...
console.log("dragend",b.target.getLatLng()),a(b.target.getLatLng())}).addTo(b),a(d.latlng)}),L.tileLayer("http://{s}.tile.osm.org/{z}/{x}/{y}.png",{attribution:"&copy; <a href=\"http://osm.org/copyright\">OpenStreetMap</a> contributors"}).addTo(b)):($("#mapContainer").hide(),b?.remove())});var d;$("#addressInput").on("input",function(a){//console.log(e, e.target.value);
function b(a){/*if (data.display_name) {
...
//}
$.each(a,function(a,b){//if (item.class !== 'boundary') {
//console.log(item);
$("#addressList").append($("<li>").text(b.display_name))})}clearTimeout(d),d=setTimeout(function(){$("#addressList").empty(),$.ajax({type:"GET",url:`https://nominatim.openstreetmap.org/search?q=${a.target.value}&format=json&accept-language=ru`,success:b})},300)})});