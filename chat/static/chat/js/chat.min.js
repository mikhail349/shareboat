class MessageHandler{constructor(a,b){this.msgContainer=$("#msgContainer"),this.btnSend=$("#btnSendMessage"),this.textArea=$("#textArea"),this.isOnBottom=!1,this.longPollingApi=a,this.sendMessageApi=b,this.lastAppendedSenderId=void 0;var c=this;$(window).scroll(function(){c.isOnBottom=$(window).scrollTop()+$(window).height()==$(document).height()}),this.textArea.on("input",function(){const a=c.getValue();c.btnSend.attr("disabled",!a)}),this.textArea.on("keydown",function(a){13===a.keyCode&&a.preventDefault()}),this.textArea.on("keyup",function(a){13===a.keyCode&&(a.preventDefault(),c.sendMessage())}),this.btnSend.on("click",function(){c.sendMessage()}),this.appendExistingMessages(),this.getNewMessages()}getValue(){return this.textArea.val()}getAvatar(a){return this.lastAppendedSenderId==a?.sender?.id?"<div style=\"width:32px; height:32px; min-width: 32px !important; min-height: 32px;\"></div>":a?.sender?.avatar?`
                <img
                    height="32"
                    width="32"
                    src="${a.sender.avatar}" 
        ...
        `}appendMessage(a){this.msgContainer.append(this.getMessageHtml(a)),this.lastAppendedSenderId=a?.sender?.id}appendExistingMessages(){if(0<window?.messages.length){for(var a of window.messages)this.appendMessage(a);$("html, body").scrollTop($(document).height())}}sendMessage(){function a(a){d.textArea.val(null),d.appendMessage(a.data),$("html, body").animate({scrollTop:$(document).height()})}function b(a){showErrorToast(a.responseJSON.message)}const c=this.getValue();if(c){const e=new FormData;e.append("text",c);var d=this;$.ajax({type:"POST",url:d.sendMessageApi,data:e,processData:!1,contentType:!1,success:a,error:b})}}// Long polling
