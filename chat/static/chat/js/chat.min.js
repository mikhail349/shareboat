class MessageHandler{constructor(a,b){this.GET_NEW_MESSAGES_TIMER=2,this.msgContainer=$("#msgContainer"),this.btnSend=$("#btnSendMessage"),this.textArea=$("#textArea"),this.btnGoToBottom=$(".btn-go-to-bottom"),this.isOnBottom=!1,this.getMessagesApi=a,this.sendMessageApi=b,this.lastAppendedSenderId=void 0;var c=this;c.isOnBottom=!1,$(window).scroll(function(){const a=Math.ceil(window.scrollY)-70;c.isOnBottom=window.innerHeight+a>=document.body.offsetHeight,c.isOnBottom?(c.btnGoToBottom.removeClass("show"),c.btnGoToBottom.addClass("hide"),$("#hasNewMessages").hide()):(c.btnGoToBottom.removeClass("hide"),c.btnGoToBottom.addClass("show"))}),this.btnGoToBottom.on("click",function(){$("html, body").animate({scrollTop:$(document).height()})}),this.textArea.on("input",function(){const a=c.getValue();c.btnSend.attr("disabled",!a)}),this.textArea.on("keydown",function(a){13===a.keyCode&&a.preventDefault()}),this.textArea.on("keyup",function(a){13===a.keyCode&&(a.preventDefault(),c.sendMessage())}),this.btnSend.on("click",function(){c.sendMessage()}),this.appendExistingMessages(),this.getNewMessages=this.getNewMessages.bind(this),this.msgTimerId=setInterval(c.getNewMessages,1e3*this.GET_NEW_MESSAGES_TIMER)}getValue(){return this.textArea.val()}getAvatar(a){return this.lastAppendedSenderId==a?.sender?.id?"<div style=\"width:32px; height:32px; min-width: 32px !important; min-height: 32px;\"></div>":a?.sender?.avatar?`
                <img
                    height="32"
                    width="32"
                    src="${a.sender.avatar}" 
