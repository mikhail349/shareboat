var isOnBottom=!1;$(document).ready(()=>{function a(a){return a?.sender?.avatar?`
                <img
                    ...
                    alt 
                />
            `:"<div style=\"width:32px; height:32px\"></div>"}function b(a){return`${("0"+a.getDate()).slice(-2)}.${("0"+(a.getMonth()+1)).slice(-2)}.${a.getFullYear()}, ${("0"+a.getHours()).slice(-2)}:${("0"+a.getMinutes()).slice(-2)}`}function c(c){const d=c.is_out?"list-group-item-primary ":"list-group-item-secondary ";return`
            <div id="msg${c.id}" class="list-group-item ${d} mb-3 rounded border pb-0 pe-0" style="width: fit-content;">
                ...
                <small class="float-end pe-1" style="font-size: 12px;">${b(new Date(c.sent_at))}</small>
            </div>  
