...
def list(request):
    ...
    # booking renter
    msgs = MessageBooking.objects.filter(
        Q(sender=user) | Q(recipient=user)).order_by('-pk')
    ...
    # boat owner
    msgs = MessageBoat.objects.filter(
        Q(sender=user) | Q(recipient=user)).order_by('-pk')
...
def send_message(request):
    if request.method == 'POST':
        data = request.POST
        new_message = MessageSupport.objects.create(
            text=data.get('text'), sender=request.user, recipient=None)
...
def boat(request, pk):
    try:
        ...
        messages_ser_data = MessageSerializerList(
            messages, many=True, context={'request': request}).data

        if is_moderator:
            messages.filter(Q(sender__isnull=True) | Q(
                recipient__isnull=True), read=False).update(read=True)
        else:
            messages.filter(recipient=request.user,
                            read=False).update(read=True)
